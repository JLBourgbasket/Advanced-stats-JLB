...
function aggregate(matches: MatchBoxscore[]) {
  ...
  const players = calculatePlayerMetrics(aggregateMatch).map((player) => {
    ...
    return { ...player, games: source.games, mpg: source.minutes / games, ppg: player.points / games, rpg: (player.orb + player.drb) / games, apg: player.ast / games };
...
export function JlHistoryPanel({ matches, onOpenMatch }: { matches: MatchBoxscore[]; onOpenMatch: (match: MatchBoxscore) => void }) {
  ...
  if (!data) return <section className="panel grid min-h-80 place-items-center p-6 text-center"><div><CalendarRange className="mx-auto size-10 text-stone-400" /><h2 className="mt-4 text-2xl font-black">{tr("Aucun historique JL Bourg", "No JL Bourg history")}</h2><p className="mt-2 text-sm text-stone-500">{tr("Classez un import comme « Match JL Bourg » pour commencer l’historique.", "Classify an import as a “JL Bourg game” to start the history.")}</p></div></section>;
  ...
  const identityMetrics = [
    { label: "TS%", value: metrics.ts },
    { label: "eFG%", value: metrics.efg },
    { label: "3P%", value: metrics.threePct },
    { label: "FGAST%", value: metrics.fgast, invalid: !metrics.fgastValid },
    { label: "TOV%", value: metrics.tov },
    { label: "ORB%", value: metrics.orb },
    { label: "DRB%", value: metrics.drb },
    { label: "OPP eFG%", value: metrics.oppEfg },
  ];

  return <div className="space-y-5">
    <section className="panel p-5 sm:p-6"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="eyebrow">{tr("Analyse multi-matchs", "Multi-game analysis")}</p><h2 className="mt-2 text-3xl font-black">{tr("Historique JL Bourg", "JL Bourg history")}</h2><p className="mt-1 text-sm text-stone-500">{tr("Les statistiques sont recalculées sur les volumes cumulés, puis ramenées au nombre de matchs lorsque nécessaire.", "Statistics are recalculated from cumulative volumes, then converted to per-game values where needed.")}</p></div><div className="text-right"><div className="font-condensed text-4xl font-black">{wins}–{losses}</div><div className="text-xs text-stone-500">{tr("bilan sur l’échantillon", "sample record")}</div></div></div><div className="print-hidden mt-5 flex flex-wrap items-center gap-2 border-t border-stone-200 pt-4"><span className="mr-2 text-xs font-bold text-stone-500">{tr("ÉCHANTILLON", "SAMPLE")}</span>{([1, 3, 5, 10] as const).map((size) => <button key={size} disabled={matches.length < size} onClick={() => setSampleSize(size)} className={`border px-3 py-1 text-xs font-bold ${sampleSize === size ? "border-stone-950 bg-stone-950 text-white" : "border-stone-300 bg-white"} ${matches.length < size ? "cursor-not-allowed opacity-35" : ""}`}>{size} {tr(size > 1 ? "matchs" : "match", size > 1 ? "games" : "game")}</button>)}<button onClick={() => setSampleSize("all")} className={`border px-3 py-1 text-xs font-bold ${sampleSize === "all" ? "border-stone-950 bg-stone-950 text-white" : "border-stone-300 bg-white"}`}>{tr("Saison", "Season")} ({matches.length})</button></div></section>

    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6"><HistoryCard label={tr("Bilan", "Record")} value={`${wins}–${losses}`} copy={`${formatMetric((100 * wins) / games)}% ${tr("victoires", "wins")}`} /><HistoryCard label={tr("Points", "Points")} value={formatMetric(pointsFor)} copy={tr("marqués par match", "scored per game")} /><HistoryCard label={tr("Encaissés", "Allowed")} value={formatMetric(pointsAgainst)} copy={tr("par match", "per game")} /><HistoryCard label="ORtg" value={formatMetric(metrics.ortg)} copy={tr("cible ≥ 115", "target ≥ 115")} /><HistoryCard label="DRtg" value={formatMetric(metrics.drtg)} copy={tr("cible ≤ 110", "target ≤ 110")} /><HistoryCard label="Net" value={`${metrics.net > 0 ? "+" : ""}${formatMetric(metrics.net)}`} copy={tr("pour 100 possessions", "per 100 possessions")} /></section>

    <section className="grid gap-5 lg:grid-cols-[1.2fr_.8fr]"><article className="panel p-5"><div className="flex items-center gap-2"><TrendingUp className="size-5 text-[#d71920]" /><div><p className="eyebrow">Match par match</p><h3 className="mt-1 text-xl font-black">Évolution des ratings</h3></div></div>{trend.length > 1 ? <div className="mt-4 h-72"><ResponsiveContainer width="100%" height="100%"><LineChart data={trend}><CartesianGrid stroke="#e7e3db" vertical={false} /><XAxis dataKey="date" axisLine={false} tickLine={false} fontSize={11} /><YAxis domain={[80, 140]} axisLine={false} tickLine={false} fontSize={11} /><Tooltip /><Legend wrapperStyle={{ fontSize: 11 }} /><ReferenceLine y={115} stroke="#d71920" strokeDasharray="4 4" /><ReferenceLine y={110} stroke="#78716c" strokeDasharray="4 4" /><Line type="monotone" dataKey="ORtg" stroke="#d71920" strokeWidth={3} /><Line type="monotone" dataKey="DRtg" stroke="#292524" strokeWidth={3} /><Line type="monotone" dataKey="Net" stroke="#d4a64a" strokeWidth={2} /></LineChart></ResponsiveContainer></div> : <div className="mt-4 grid h-72 place-items-center border border-dashed border-stone-300 bg-stone-50 text-center text-sm text-stone-500">Sélectionnez au moins trois matchs lorsque l’historique le permettra.</div>}</article><article className="panel p-5"><div className="flex items-center gap-2"><Gauge className="size-5 text-[#d71920]" /><div><p className="eyebrow">Identité de l’échantillon</p><h3 className="mt-1 text-xl font-black">Indicateurs clés</h3></div></div><div className="mt-4 grid grid-cols-2 gap-3">{identityMetrics.map(({ label, value, invalid }) => <div key={label} className={`border p-3 ${invalid ? "border-amber-200 bg-amber-50" : "border-stone-200 bg-stone-50"}`}><div className="text-[10px] font-bold text-stone-400">{label}</div><div className="mt-1 text-xl font-black">{invalid ? "À contrôler" : `${formatMetric(value)}%`}</div>{invalid && <div className="mt-1 text-[9px] text-amber-700">brut {formatMetric(metrics.fgastRaw)}%</div>}</div>)}</div></article></section>

    <section className="panel overflow-hidden"><div className="flex items-center gap-3 border-b border-stone-200 p-5"><Users className="size-5 text-[#d71920]" /><div><p className="eyebrow">Cumul de l’échantillon</p><h3 className="mt-1 text-xl font-black">Production individuelle historique</h3></div></div><div className="overflow-x-auto"><Table className="min-w-[1300px]"><TableHeader><TableRow className="bg-stone-50"><TableHead>Joueur</TableHead>{["MJ", "MIN", "PTS", "REB", "USG%", "TS%", "eFG%", "AST/M", "AST% estim.", "TOV%", "ORB%", "TRB%", "PTS/40", "AST/40"].map((label) => <TableHead key={label} className="text-right">{label}</TableHead>)}</TableRow></TableHeader><TableBody>{data.players.map((player) => <TableRow key={player.id}><TableCell className="font-bold">{player.name}</TableCell><TableCell className="text-right">{player.games}</TableCell><TableCell className="text-right">{formatMetric(player.mpg)}</TableCell><TableCell className="text-right font-bold">{formatMetric(player.ppg)}</TableCell><TableCell className="text-right">{formatMetric(player.rpg)}</TableCell><TableCell className="text-right">{formatMetric(player.usg)}</TableCell><TableCell className="text-right">{formatMetric(player.ts)}</TableCell><TableCell className="text-right">{formatMetric(player.efg)}</TableCell><TableCell className="text-right font-bold">{formatMetric(player.apg)}</TableCell><TableCell className={`text-right ${player.astPctLowSample ? "bg-amber-50 text-amber-800" : ""}`} title={`AST% estimé sur ${formatMetric(player.estimatedTeammateFieldGoals, 2)} paniers de coéquipiers`}><span aria-hidden="true">~</span>{formatMetric(player.astPct)}{player.astPctLowSample ? " ⚠" : ""}</TableCell><TableCell className="text-right">{formatMetric(player.tovPct)}</TableCell><TableCell className="text-right">{formatMetric(player.orbPct)}</TableCell><TableCell className="text-right">{formatMetric(player.trbPct)}</TableCell><TableCell className="text-right">{formatMetric(player.pts40)}</TableCell><TableCell className="text-right">{formatMetric(player.ast40)}</TableCell></TableRow>)}</TableBody></Table></div><p className="border-t border-stone-200 p-4 text-[11px] text-stone-500">AST/M et AST/40 sont les repères de volume. AST% estim. devient plus robuste à mesure que l’échantillon cumule les matchs.</p></section>

    <section className="panel overflow-hidden"><div className="flex items-center gap-3 border-b border-stone-200 p-5"><BarChart3 className="size-5 text-[#d71920]" /><div><p className="eyebrow">{tr("Accès aux rapports", "Report access")}</p><h3 className="mt-1 text-xl font-black">{tr("Matchs de l’échantillon", "Games in sample")}</h3></div></div><div className="grid gap-px bg-stone-200 sm:grid-cols-2 xl:grid-cols-3">{sample.map((match) => <button key={match.id} onClick={() => onOpenMatch(match)} className="bg-white p-4 text-left hover:bg-red-50"><div className="flex items-center justify-between gap-3"><span className="font-black">vs {match.opponent.name}</span><span className={`font-condensed text-lg font-black ${match.team.points > match.opponent.points ? "text-emerald-700" : "text-[#d71920]"}`}>{match.team.points}–{match.opponent.points}</span></div><div className="mt-2 text-xs text-stone-500">{match.date.split("-").reverse().join("/")} · {match.competition}</div></button>)}</div></section>
  </div>;
}
