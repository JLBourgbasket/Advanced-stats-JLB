...
function buildSignals(metrics: TeamMetrics, tr: (fr: string, en: string) => string) {
  const positives: Array<{ title: string; detail: string; score: number }> = [];
  const workOns: Array<{ title: string; detail: string; score: number }> = [];
  if (metrics.drtg <= 110) positives.push({ title: tr("Défense dans la cible", "Defense on target"), detail: tr(`${formatMetric(metrics.drtg)} points encaissés pour 100 possessions estimées.`, `${formatMetric(metrics.drtg)} points allowed per 100 estimated possessions.`), score: 110 - metrics.drtg });
  if (metrics.efg >= 54.5) positives.push({ title: tr("Qualité de tirs", "Shot quality"), detail: tr(`${formatMetric(metrics.efg)}% d’eFG, au-dessus de la cible collective.`, `${formatMetric(metrics.efg)}% eFG, above the team target.`), score: metrics.efg - 54.5 });
  if (metrics.orb >= 33) positives.push({ title: tr("Présence au rebond offensif", "Offensive rebounding presence"), detail: tr(`${formatMetric(metrics.orb)}% des rebonds disponibles captés.`, `${formatMetric(metrics.orb)}% of available rebounds secured.`), score: metrics.orb - 33 });
  if (metrics.fgastValid && metrics.fgast >= 62) positives.push({ title: tr("Création collective", "Team creation"), detail: tr(`${formatMetric(metrics.fgast)}% des paniers de champ sont assistés.`, `${formatMetric(metrics.fgast)}% of field goals are assisted.`), score: metrics.fgast - 62 });
  if (!metrics.fgastValid) workOns.push({ title: tr("Donnée de création à contrôler", "Creation data to review"), detail: tr(`Ratio brut ${formatMetric(metrics.fgastRaw)}% : davantage de passes décisives que de paniers réussis dans la source.`, `Raw ratio ${formatMetric(metrics.fgastRaw)}%: the source has more assists than made field goals.`), score: 100 });
  if (metrics.tov > 14.5) workOns.push({ title: tr("Sécuriser la balle", "Protect the ball"), detail: tr(`${formatMetric(metrics.tov)}% de pertes de balle : cible ≤ 14,5%.`, `${formatMetric(metrics.tov)}% turnover rate: target ≤ 14.5%.`), score: metrics.tov - 14.5 });
  if (metrics.ts < 59) workOns.push({ title: tr("Rendement global", "Overall efficiency"), detail: tr(`${formatMetric(metrics.ts)}% de TS contre une cible de 59%.`, `${formatMetric(metrics.ts)}% TS against a 59% target.`), score: 59 - metrics.ts });
  if (metrics.drb < 70) workOns.push({ title: tr("Finir les possessions", "Finish possessions"), detail: tr(`${formatMetric(metrics.drb)}% de rebonds défensifs sécurisés.`, `${formatMetric(metrics.drb)}% of defensive rebounds secured.`), score: 70 - metrics.drb });
  if (metrics.oppEfg > 52) workOns.push({ title: tr("Contestation des tirs", "Shot contests"), detail: tr(`L’adversaire atteint ${formatMetric(metrics.oppEfg)}% d’eFG.`, `The opponent reached ${formatMetric(metrics.oppEfg)}% eFG.`), score: metrics.oppEfg - 52 });
  if (positives.length === 0) positives.push({ title: tr("Base de travail", "Starting point"), detail: tr("Aucune cible collective n’est nettement dépassée sur ce match.", "No team target was clearly exceeded in this game."), score: 0 });
  if (workOns.length === 0) workOns.push({ title: tr("Match maîtrisé", "Controlled game"), detail: tr("Aucun indicateur majeur n’est nettement sous la cible.", "No major metric is clearly below target."), score: 0 });
  return { positives: positives.sort((a, b) => b.score - a.score).slice(0, 3), workOns: workOns.sort((a, b) => b.score - a.score).slice(0, 3) };
}

function DetailedPlayers({ players, tr }: { players: PlayerMetrics[]; tr: (fr: string, en: string) => string }) {
  return <section className="panel overflow-hidden"><div className="flex items-center gap-3 border-b border-stone-200 p-5"><Users className="size-5 text-[#d71920]" /><div><p className="eyebrow">{tr("Match sélectionné", "Selected game")}</p><h3 className="mt-1 text-xl font-black">{tr("Contribution individuelle complète", "Complete individual contribution")}</h3></div></div><div className="overflow-x-auto"><Table className="min-w-[1750px]"><TableHeader><TableRow className="bg-stone-50"><TableHead>{tr("Joueur", "Player")}</TableHead>{["MIN", "PTS", "REB", "USG%", "TS%", "eFG%", "2P%", "3P%", "3PAr", "FTr", "AST", tr("AST% estim.", "est. AST%"), "TOV%", "ORB%", "DRB%", "TRB%", "PTS/40", "REB/40", "AST/40", "ORtg*", "DRtg*", "+/−"].map((label) => <TableHead key={label} className="text-right">{label}</TableHead>)}</TableRow></TableHeader><TableBody>{players.map((player) => { const twoPm = player.fgm - player.threePm; const twoPa = player.fga - player.threePa; return <TableRow key={player.id}><TableCell><div className="font-bold">{player.name}</div><div className="text-[10px] text-stone-400">{player.role}</div></TableCell><TableCell className="text-right">{player.minutes}</TableCell><TableCell className="text-right font-bold">{player.points}</TableCell><TableCell className="text-right">{player.orb + player.drb}</TableCell><TableCell className="text-right">{formatMetric(player.usg)}</TableCell><TableCell className="text-right">{formatMetric(player.ts)}</TableCell><TableCell className="text-right">{formatMetric(player.efg)}</TableCell><TableCell className="text-right">{formatMetric(pct(twoPm, twoPa))}</TableCell><TableCell className="text-right">{formatMetric(pct(player.threePm, player.threePa))}</TableCell><TableCell className="text-right">{formatMetric(player.threePar)}</TableCell><TableCell className="text-right">{formatMetric(player.ftr)}</TableCell><TableCell className="text-right font-bold">{player.ast}</TableCell><TableCell className={`text-right ${player.astPctLowSample ? "bg-amber-50 text-amber-800" : ""}`} title={tr(`AST% estimé sur ${formatMetric(player.estimatedTeammateFieldGoals, 2)} paniers de coéquipiers`, `AST% estimated from ${formatMetric(player.estimatedTeammateFieldGoals, 2)} teammate field goals`)}><span aria-hidden="true">~</span>{formatMetric(player.astPct)}{player.astPctLowSample ? " ⚠" : ""}</TableCell><TableCell className="text-right">{formatMetric(player.tovPct)}</TableCell><TableCell className="text-right">{formatMetric(player.orbPct)}</TableCell><TableCell className="text-right">{formatMetric(player.drbPct)}</TableCell><TableCell className="text-right">{formatMetric(player.trbPct)}</TableCell><TableCell className="text-right">{formatMetric(player.pts40)}</TableCell><TableCell className="text-right">{formatMetric(player.reb40)}</TableCell><TableCell className="text-right">{formatMetric(player.ast40)}</TableCell><TableCell className="text-right">{formatMetric(player.ortgEstimate)}</TableCell><TableCell className="text-right">{formatMetric(player.drtgEstimate)}</TableCell><TableCell className="text-right">{player.plusMinus > 0 ? "+" : ""}{player.plusMinus}</TableCell></TableRow>; })}</TableBody></Table></div><p className="border-t border-stone-200 p-4 text-[11px] leading-5 text-stone-500">{tr("AST% estim. est contextualisé par les minutes sans rotations réelles ; « ~ » signale l’estimation et ⚠ un faible nombre d’opportunités. Sur un match isolé, privilégiez AST et AST/40. * ORtg/DRtg individuels = estimations boxscore, et non ratings on-court.", "Estimated AST% is contextualized by minutes without actual rotation data; “~” marks the estimate and ⚠ a low number of opportunities. For a single game, prioritize AST and AST/40. * Individual ORtg/DRtg are boxscore estimates, not on-court ratings.")}</p></section>;
}
...
export function JlPerformanceDeepDive({ match, metrics, history }: { match: MatchBoxscore; metrics: TeamMetrics; history: MatchBoxscore[] }) {
  ...
  const signals = buildSignals(metrics, tr);
  ...
  return <div className="space-y-5">
    <section><div className="mb-3 flex flex-wrap items-end justify-between gap-3"><div><p className="eyebrow">JL Bourg deep dive</p><h2 className="mt-1 text-xl font-black">{tr("Lecture avancée complète", "Complete advanced analysis")}</h2></div><p className="text-xs text-stone-500">{tr(`Match courant + historique des ${relevantHistory.length} derniers matchs disponibles`, `Current game + history of the latest ${relevantHistory.length} available games`)}</p></div><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><RatingCard label="ORtg" value={formatMetric(metrics.ortg)} copy={tr("Cible collective ≥ 115", "Team target ≥ 115")} tone={metrics.ortg >= 115 ? "good" : metrics.ortg >= 110 ? "watch" : "bad"} /><RatingCard label="DRtg" value={formatMetric(metrics.drtg)} copy={tr("Cible collective ≤ 110", "Team target ≤ 110")} tone={metrics.drtg <= 110 ? "good" : metrics.drtg <= 115 ? "watch" : "bad"} /><RatingCard label="Net" value={`${metrics.net > 0 ? "+" : ""}${formatMetric(metrics.net)}`} copy={tr("Cible collective ≥ +7", "Team target ≥ +7")} tone={metrics.net >= 7 ? "good" : metrics.net >= 0 ? "watch" : "bad"} /><RatingCard label={tr("Rythme", "Pace")} value={formatMetric(pace)} copy={tr("Possessions estimées", "Estimated possessions")} tone="neutral" /></div></section>

    <section className="grid gap-5 lg:grid-cols-2"><article className="panel p-5"><div className="flex items-center gap-2"><CheckCircle2 className="size-5 text-emerald-600" /><div><p className="eyebrow">{tr("Ce qui fonctionne", "What works")}</p><h3 className="mt-1 text-xl font-black">{tr("Points d’appui", "Strengths")}</h3></div></div><div className="mt-4 space-y-2">{signals.positives.map((signal, index) => <div key={signal.title} className="flex gap-3 border-l-4 border-emerald-500 bg-emerald-50 p-4"><span className="grid size-7 shrink-0 place-items-center bg-white text-xs font-black">{index + 1}</span><div><div className="text-sm font-black text-emerald-950">{signal.title}</div><p className="mt-1 text-xs leading-5 text-emerald-800">{signal.detail}</p></div></div>)}</div></article><article className="panel p-5"><div className="flex items-center gap-2"><AlertTriangle className="size-5 text-[#d71920]" /><div><p className="eyebrow">{tr("Priorités staff", "Staff priorities")}</p><h3 className="mt-1 text-xl font-black">{tr("Axes de progression", "Areas for improvement")}</h3></div></div><div className="mt-4 space-y-2">{signals.workOns.map((signal, index) => <div key={signal.title} className="flex gap-3 border-l-4 border-[#d71920] bg-red-50 p-4"><span className="grid size-7 shrink-0 place-items-center bg-white text-xs font-black">{index + 1}</span><div><div className="text-sm font-black text-red-950">{signal.title}</div><p className="mt-1 text-xs leading-5 text-red-800">{signal.detail}</p></div></div>)}</div></article></section>
    ...
    <DetailedPlayers players={players} tr={tr} />
