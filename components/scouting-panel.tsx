...
export function ScoutingPanel({ matches, selectedId, onSelect, canDelete = false, onDelete }: { matches: MatchBoxscore[]; selectedId: string; onSelect: (id: string) => void; canDelete?: boolean; onDelete?: (match: MatchBoxscore) => void }) {
  ...
  return <div className="grid gap-5 xl:grid-cols-[280px_minmax(0,1fr)]">
    <aside className="panel print-hidden h-fit p-4 xl:sticky xl:top-5"><p className="eyebrow">Rapports disponibles</p>{!canDelete && <p className="mt-2 text-[10px] leading-4 text-stone-400">La suppression apparaît après connexion administrateur dans l’onglet Données.</p>}<div className="mt-4 max-h-[70vh] space-y-2 overflow-y-auto">{matches.map((item) => <div key={item.id} className={`flex items-start border ${item.id === match.id ? "border-[#d71920] bg-red-50" : "border-stone-200 hover:bg-stone-50"}`}><button type="button" onClick={() => { setSampleSize(1); onSelect(item.id); }} className="min-w-0 flex-1 p-3 text-left"><div className="font-bold">{item.team.name}</div><div className="mt-1 text-xs text-stone-500">vs {item.opponent.name} · {item.date.split("-").reverse().join("/")}</div></button>{canDelete && onDelete && <div className="p-1.5"><DeleteMatchButton label={`${item.team.name} contre ${item.opponent.name}`} onDelete={() => onDelete(item)} /></div>}</div>)}</div></aside>
